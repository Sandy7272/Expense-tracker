...
export async function parsePDF(file: File): Promise<ParsedTransaction[]> {
  // For PDF parsing, we'll extract text and use pattern matching
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
    ...
    reader.readAsText(file); // Simple text extraction
  });
}
...
function extractTransactionsFromText(text: string): ParsedTransaction[] {
  ...
  const transactionPattern = /(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})\s+(.+?)\s+(?:Rs\.?|INR|₹)?\s*([\d,]+\.?\d*)/gi;
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      transactions.push({
        ...
        type: 'expense' // Default, will be refined by AI
      });
    }
  }
